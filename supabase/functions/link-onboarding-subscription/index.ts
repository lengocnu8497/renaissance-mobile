...
serve(async (req) => {
  ...
  try {
    ...
    const tier = subscription.metadata?.tier ?? 'silver'

    // billing_plan mirrors tier directly now that BillingPlan supports 'annual'
    const billingPlan = tier === 'silver' ? 'silver' : tier === 'annual' ? 'annual' : 'gold'
