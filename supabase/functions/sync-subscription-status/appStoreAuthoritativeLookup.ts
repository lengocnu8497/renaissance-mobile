...
import { tierForAppStoreProductId, type AppStoreSubscriptionTier } from '../_shared/appStoreProducts.ts'
...
export function decodeSignedTransactionPayload(
  ...
  return decodeSegment<DecodedTransactionPayload>(segments[1])
}
  ...
  if (input.transactionId) {
    ...
    if (transactionResponse.signedTransactionInfo) {
      transactionCandidate = {
        verifiedTransaction: parseVerifiedTransactionPayload(
          decodeSignedTransactionPayload(transactionResponse.signedTransactionInfo),
...
function selectBestSubscriptionStatus(
  ...
  const candidates = (response.data ?? [])
    ...
    .map((item) => {
      ...
      const verifiedTransaction = parseVerifiedTransactionPayload(
        decodeSignedTransactionPayload(item.signedTransactionInfo),
...
function parseVerifiedTransactionPayload(
...
function decodeSignedRenewalInfoPayload(
  signedRenewalInfo: string,
): DecodedRenewalInfoPayload {
  const segments = signedRenewalInfo.split('.')
  if (segments.length !== 3) {
    throw new AppStoreSignedTransactionVerificationError('App Store signed payload must be a compact JWS')
  }

  return decodeSegment<DecodedRenewalInfoPayload>(segments[1])
}
